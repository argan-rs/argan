...
pub(crate) struct FromSegment<'p, 'de> {
	segment_name: &'de str,
...
impl<'p, 'de> FromSegment<'p, 'de> {
	...
	pub(super) fn deserialize_map_keey<K>(&mut self, seed: K) -> Result<Option<K::Value>, E>
	where
	...
	pub(super) fn deserialize_map_value<K>(&mut self, seed: K) -> Result<K::Value, E>
	where
		K: DeserializeSeed<'de>,
	{
		let mut deserializer = FromSegmentParamsMapAccess::new(self);
...
impl<'p, 'de> Iterator for FromSegment<'p, 'de> {
...
impl<'p, 'de> Deserializer<'de> for &mut FromSegment<'p, 'de> {
	type Error = E;
...
struct FromSegmentParamsSeqAccess<'a, 'p, 'de>(&'a mut FromSegment<'p, 'de>);

impl<'a, 'p, 'de> FromSegmentParamsSeqAccess<'a, 'p, 'de> {
	#[inline]
	fn new(from_segment_params: &'a mut FromSegment<'p, 'de>) -> Self {
...
struct FromSegmentParamsMapAccess<'a, 'p, 'de>(&'a mut FromSegment<'p, 'de>);
...
struct FromSegmentParamsEnumAccess<'a, 'p, 'de>(&'a mut FromSegment<'p, 'de>);
